...
def make_csv(folder_name, common_species, common_variety,disease,rot, string):
    elements = string.split('_')
    elements = [element.replace('0', '') for element in elements]
    elements = [folder_name, common_species, common_variety, disease,rot] + elements
...
def main(folder_name,common_species, common_variety, disease,rot, input_name):
    ...
    if found_string is not None:
        ...
        print(output)
        make_csv(folder_name, common_species, common_variety, disease,rot,  output)
    else:
...
def main_batch(file_path):
    with open(file_path, "r", encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            rot = row.get('rot', '')
            if species:
                main(folder_name, common_species, common_variety,disease,rot, species.strip())
            else:
...
if __name__ == '__main__':
    taxonomy = []
    name_dataset_information = 'D:/Dataset processing/Fruit-262/Fruit-262_dataset_information.csv'
    get_taxonomy = 'D:/Dataset processing/Fruit-262/Fruit-262_taxonomy.csv'
    ...
    with open(get_taxonomy, 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['folder_name','common_species', 'common_variety','disease','rot', 'kingdom', 'phylum', 'class', 'order', 'family', 'subfamily', 'genus', 'species'])
