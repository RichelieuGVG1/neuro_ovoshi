...
def add_to_shard(image_path: str, dataset_name: str, shard_root: str, taxonomy_df: pd.DataFrame) -> None:
    ...
    species_folder = os.path.basename(os.path.dirname(image_path)).strip().lower()
    taxonomy_row = None
    if 'folder_name' in taxonomy_df.columns:
        taxonomy_row = taxonomy_df[taxonomy_df['folder_name'].str.strip().str.lower() == species_folder]
    if taxonomy_row is None or taxonomy_row.empty:
        if 'common_species' in taxonomy_df.columns:
            taxonomy_row = taxonomy_df[taxonomy_df['common_species'].str.strip().str.lower() == species_folder]
...
if __name__ == "__main__":
    dataset_name = "Fruit-262"
    shard_root = "0000"
    taxonomy_file = "D:/Dataset processing/Fruit-262/Fruit-262_taxonomy.csv"
    image_dir = "D:/Dataset processing/Fruit-262"
